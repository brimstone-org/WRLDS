...
namespace _DPS
{
    ...
    public class NodeAttach : MonoBehaviour {
        ...
        void OnEnable()
        {
            yOffset = Inversed ? -yOffset : yOffset;
        }
        void Update() {
                ...
                if ((spline.GetPointCount() != 0) && (lastSpritePointCount != 0)) {
                    ...
                    if ((index <= spline.GetPointCount() - 1) && (index >= 0)) {
                        if (useNormals) {
                            ...
                            transform.rotation = Inversed ? Quaternion.Euler(0, 0, 180) : Quaternion.Euler(0, 0, 0);
                        }
                        ...
                        transform.position = spriteShapeController.transform.position + spline.GetPosition(index) + offsetVector + new Vector3(xOffset, 0, 0);
