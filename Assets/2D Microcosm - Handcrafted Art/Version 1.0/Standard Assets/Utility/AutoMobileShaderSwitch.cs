...
namespace UnityStandardAssets.Utility
{
    public class AutoMobileShaderSwitch : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
			...
			global::Logger.Log (renderers.Length+" renderers");
				...
				foreach(var r in renderers)
				{
					...
					for(int n=0; n<r.sharedMaterials.Length; ++n)
					{
						...
						if (material.shader == replacementDef.original)
						{
							...
							global::Logger.Log ("replacing "+r.gameObject.name+" renderer "+n+" with "+newMaterials[oldMaterials.IndexOf(material)].name);
			...
			global::Logger.Log (materialInstancesReplaced+" material instances replaced");
			global::Logger.Log (materialsReplaced+" materials replaced");
			for(int n=0; n<oldMaterials.Count; ++n)
			{
				global::Logger.Log (oldMaterials[n].name+" ("+oldMaterials[n].shader.name+")"+" replaced with "+newMaterials[n].name+" ("+newMaterials[n].shader.name+")");
			}
#endif
        }
