	...
	public Boolean UseTimeScale = true;
        ...
        internal void InitialiseChildObjects()
        {
            if (ChildObjects != null && ChildObjects.Length > 0)
            {
                ...
                {
                    //global::Logger.LogError("ScrollRect Content has children, this is not supported when using managed Child Objects\n Either remove the ScrollRect Content children or clear the ChildObjects array");
                    return;
                }
        ...
        internal void UpdateVisible()
        {
            ...
            for (int i = CurrentPage - _bottomItem; i < CurrentPage + _topItem; i++)
            {
                ...
                {
                    global::Logger.Log("Failed to setactive child [" + i + "]");
                }
            }
        ...
        private void OnValidate()
        {
            ...
            {
                //global::Logger.LogError("ScrollRect has to have a direction, please select either Horizontal OR Vertical with the appropriate control.");
            }
            if (_scroll_rect.horizontal && _scroll_rect.vertical)
            {
                //global::Logger.LogError("ScrollRect has to be unidirectional, only use either Horizontal or Vertical on the ScrollRect, NOT both.");
            }
