    ...
    public class ScrollConflictManager : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        void Awake()
        {
            ...
            if (scrollOtherHorizontally)
            {
                ...
                {
                    global::Logger.Log("You have added the SecondScrollRect to a scroll view that already has both directions selected");
                }

                if (!ParentScrollRect.horizontal){
                    global::Logger.Log("The other scroll rect doesnt support scrolling horizontally");
                }
            }
            else if (!ParentScrollRect.vertical)
            {
                global::Logger.Log("The other scroll rect doesnt support scrolling vertically");
            }
        }
